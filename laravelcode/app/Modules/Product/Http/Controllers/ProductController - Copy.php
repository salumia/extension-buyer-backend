 ...
 /*------------------------------status-sold--------------------------------------------*/
    ...
    public function productView($id, Request $request){
        ...
        if($user and $product['user_id'] == auth()->user()->id){
            $offers = DB::table('offers')
                        ->select('offers.id as offer_id','offers.awarded','users.name as first_name', 'users.last_name','users.username', 'users.image_path', 'users.country_id', 'users.state_id',  'users.city_id','offers.offered_amount')
            ...
            $buyer = DB::table('users as t1')->select('t1.id as id','t1.name  as name','t1.image_path as image','t2.name as country', 't2.sortname as country_code','t1.username as username')
    ...
    public function getBuyerListings(Request $request){
        ...
        $products = DB::table('products')
                        ->select('products.id as id', 'products.product_name as name','products.product_type as type','offers.offered_amount','offers.created_at')
        ...
        if(count($products)>0){
            ...
            foreach($products as $key => $product){
                ...
                $seller=User::select('id as seller_id','name as seller_name','username as seller_username')->where('id','=',$userId)->first();
        ...
        if(count($products1)>0){
            foreach($products1 as $key => $product1){
                ...
                $seller1=User::select('id as seller_id','name as seller_name','username as seller_username')->where('id','=',$userId)->first();
        ...
        if(count($products2)>0){
            ...
            foreach($products2 as $key => $product2){
                ...
                $seller2=User::select('id as seller_id','name as seller_name','username as seller_username')->where('id','=',$userId)->first();
